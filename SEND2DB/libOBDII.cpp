...
char * libOBDII::tratarPID(char *infoPID, char *pid, int bytes){
			...
			if(rpm != 0){
				...
				if(engine!=0){
					...
					if(maf!=99){
						...
						printf("\n");
						system("printf \"%b\" \"\e[1;34mNo disponible PID 10: MAF\e[0m\"");
					}
				}
				else{
					printf("\n");
					system("printf \"%b\" \"\e[1;31mSin carga de motor\e[0m\"");
				}
			}
			else{
				printf("\n");
				system("printf \"%b\" \"\e[1;31mAutomovil no arrancado\e[0m\"");
			}
		}
		...
		*/
		...
		if(strstr(charpid,"FE")!=NULL){
			...
			if(ctemperature>=50){
				...
				printf("\n");
				system("printf \"%b\" \"\e[1;31mTemperatura del motor por debajo de 50 Grados\nNo empieza el Juego!\e[0m\"");
			}
			if(nummuestras!=0){
   ...
   /* No contiene la informacion */
		...
		printf("\n");
		system("printf \"%b\" \"\e[1;31mPID sin informacion\e[0m\"");
	}
